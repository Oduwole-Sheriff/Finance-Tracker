        ...
        if not User.objects.filter(username=username).exists():
            if not User.objects.filter(email=email).exists():
                ...
                email = EmailMessage(
                    email_subject,
                    email_body,
                    'oduwolesheriff1212@gmail.come',
                    [email],
                )
                EmailThread(email).start()
                messages.success(request, 'Account Successfully Created, a verification link as been sent to your email')
...
class CompletePasswordReset(View):
    def get(self, request, uidb64, token):
        ...
        try:
            ...
            if not PasswordResetTokenGenerator().check_token(user, token):
                messages.info(request, 'Password link is invalid, please request a now one')
                return render(request, 'authentication/reset-password.html')
    ...
    def post(self, request, uidb64, token):
        ...
        try:
            ...
            messages.success(request, 'Password reset successfully, you can log in with the new password')
            return redirect('login')
        except Exception as identifire:
            # import pdb
            # pdb.set_trace()
